...
export class Yt extends Token {
  ...
  public static find(address: string, chainId?: number): Yt {
    ...
    const marketInfo: PENDLEMARKETNFO | undefined = markets.find(
      (m: PENDLEMARKETNFO) => isSameAddress(m.pair[0], address) || isSameAddress(m.pair[1], address)
    );
  ...
  public yieldContract(chainId?: number): YieldContract {
    const networkInfo: NetworkInfo = distributeConstantsByNetwork(chainId);
    const ytInfo: YTINFO[] = networkInfo.contractAddresses.YTs.filter((yt: YTINFO) =>
      isSameAddress(yt.address, this.address)
    );
    if (ytInfo.length === 0) {
    ...
    return new YieldContract(
      ...
      new Token(ytInfo[0].underlyingAssetAddress, networkInfo.decimalsRecord[ytInfo[0].rewardTokenAddresses[0]]),
      this.expiry!,
      chainId
    );
  }
