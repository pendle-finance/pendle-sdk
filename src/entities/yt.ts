...
export class Yt extends Token {
  ...
  public static find(address: string, chainId?: number): Yt {
    ...
    const ytInfo: YTINFO | undefined = networkInfo.contractAddresses.YTs.find(
      (y: YTINFO) => {
    ...
    const markets: PENDLEMARKETNFO[] | undefined =
      networkInfo.contractAddresses.pendleMarkets;
    const marketInfo: PENDLEMARKETNFO | undefined = markets.find(
      (m: PENDLEMARKETNFO) =>
  ...
  public yieldContract(chainId?: number): YieldContract {
    ...
    const ytInfo: YTINFO[] = networkInfo.contractAddresses.YTs.filter(
      (yt: YTINFO) => isSameAddress(yt.address, this.address)
    );
    if (ytInfo.length === 0) {
