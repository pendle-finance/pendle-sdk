...
export class Comptroller {
  ...
  public methods({ signer, provider, chainId }: ChainSpecifics): Record<string, any> {
    ...
    const comptrollerContract: Contract = new Contract(
      networkInfo.contractAddresses.misc.Comptroller,
      getComptrollerABI(this.protocol).abi,
      provider
    );
    ...
    const getSupplierAprs = async (qiOrCToken: Token): Promise<AprInfo[]> => {
      switch (this.protocol) {
        case CompoundFork.Benqi: {
          ...
          promises.push(
            fetchValuation(QiRewardPerDay, provider, chainId).then(
          ...
          promises.push(
            fetchValuation(AvaxRewardPerDay, provider, chainId).then(
          ...
          promises.push(
            fetchValuation(totalQiTokenSupply, provider, chainId).then(
