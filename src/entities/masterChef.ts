  ...
  public static methods(
    ...
    const getRewardsAprs = async (pid: number): Promise<AprInfo[]> => {
      ...
      if (chainId == 1 || chainId === undefined) {
        const masterChefAddress: string =
          networkInfo.contractAddresses.misc.MasterChef;
        const masterChefContract: Contract = new Contract(
          masterChefAddress,
          contracts.SushiMasterChef.abi,
          signer.provider
        );
        ...
        const sushiPerBlock: BN = globalSushiPerBlock
          .mul(allocPoint)
          .div(totalAllocPoint);
        const sushiAddress: string = networkInfo.contractAddresses.tokens.SUSHI;
        const sushiPrice: BigNumber = await fetchTokenPrice({
        ...
        const lpContract: Contract = new Contract(
          poolInfo.lpToken,
        ...
        const stakedUSDValue: CurrencyAmount = await fetchValuation(
          new TokenAmount(
            new Token(
              poolInfo.lpToken,
              networkInfo.decimalsRecord[poolInfo.lpToken]
            ),
        ...
        aprs.push({
          ...
          apr: calcLMRewardApr(
            sushiRewardsValue,
            new BigNumber(stakedUSDValue.amount),
            365
          ).toFixed(DecimalsPrecision),
        });
        return aprs;
      } else if (chainId == 43114) {
        const masterChefV2Address: string =
          networkInfo.contractAddresses.misc.JOE_MASTERCHEFV2;
        const masterChefV2: Contract = new Contract(
          masterChefV2Address,
          contracts.JoeMasterChef.abi,
          signer.provider
        );
        ...
        const joeTokenAddress: string = (
          await masterChefV2.joe()
        ).toLowerCase();
        ...
        const joePrice: BigNumber = await fetchTokenPrice({
          address: joeAddress,
          signer,
          chainId,
        });
        ...
        const totalStakedLp: BN = await lpContract.balanceOf(
          masterChefV2Address
        );
        const stakedUSDValue: CurrencyAmount = await fetchValuation(
          new TokenAmount(
