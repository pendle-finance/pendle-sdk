...
export class OneClickWrapper {
  ...
  public methods(
    ...
    const forgeAddress =
      networkInfo.contractAddresses.forges[this.yieldContract.forgeIdInBytes];
    ...
    const isUnderlyingLP = (): boolean => {
      return (
        ...
        this.yieldContract.forgeIdInBytes ==
          forgeIdsInBytes.SUSHISWAP_COMPLEX ||
    ...
    const getAddOtLiqTxn = (
      ...
      return txns.find((txn: Transaction) => {
        return (
          ...
          txn.paid.filter((t: TokenAmount) =>
            isSameAddress(t.token.address, pendleFixture.ot.address)
          ).length > 0
        );
      });
    };

    const getAddYtLiqTxn = (
      txns: Transaction[],
      pendleFixture: PendleFixture
    ): Transaction | undefined => {
      return txns.find((txn: Transaction) => {
        return (
          txn.action == TransactionAction.addLiquidity &&
          txn.paid.filter((t: TokenAmount) =>
            isSameAddress(t.token.address, pendleFixture.yt.address)
          ).length > 0
        );
      });
    };
    ...
    const getPendleFixture = async (): Promise<PendleFixture> => {
      ...
      const ytMarketAddress: string = networkInfo.contractAddresses.pendleMarkets.find(
        (m: PENDLEMARKETNFO) => {
          return (
            isSameAddress(m.pair[0], ytAddress) ||
            isSameAddress(m.pair[1], ytAddress)
          );
        }
      )!.address;
      const ytMarket: PendleMarket = PendleMarket.find(
        ytMarketAddress,
        chainId
      );
      const otMarketAddress: string = networkInfo.contractAddresses.otherMarkets!.find(
        (m: MARKETINFO) => {
          return (
            isSameAddress(m.pair[0], otAddress) ||
            isSameAddress(m.pair[1], otAddress)
          );
        }
      )!.address;
      const otMarket: Market = Market.find(otMarketAddress, chainId);
      const ytStakingPoolInfo: LMINFO = networkInfo.contractAddresses.stakingPools.find(
        (lm: LMINFO) => {
          return (
            isSameAddress(lm.inputTokenAddress, ytMarketAddress) && lm.active
          );
        }
      )!;
      ...
      const otStakingPoolInfo: LMINFO = networkInfo.contractAddresses.stakingPools.find(
        (lm: LMINFO) => {
          return (
            isSameAddress(lm.inputTokenAddress, otMarketAddress) && lm.active
          );
        }
      )!;
    ...
    const scaleSimulationResult = async (
      ...
      const otMarketAddLiqTransaction: Transaction | undefined = getAddOtLiqTxn(
        transactions,
        pendleFixture
      );
      if (otMarketAddLiqTransaction === undefined) {
        ...
        otPoolShare = calcShareOfPool(
          otMarketSupply,
          BN.from(otMarketAddLiqTransaction.received[0].rawAmount())
        );
      }

      const ytMarketAddLiqTransaction: Transaction | undefined = getAddYtLiqTxn(
        transactions,
        pendleFixture
      );
      if (ytMarketAddLiqTransaction === undefined) {
        ...
        ytPoolShare = calcShareOfPool(
          ytMarketSupply,
          BN.from(ytMarketAddLiqTransaction.received[0].rawAmount())
        );
      }
    ...
    const simulateWithFixedInput = async (
      ...
      switch (this.yieldContract.forgeIdInBytes) {
        ...
        case forgeIdsInBytes.JOE_SIMPLE:
          const underlyingLp: Market = Market.find(
            this.yieldContract.underlyingAsset.address,
            chainId
          );
          ...
          const testInutTokenIdxInLP: number = isSameAddress(
            ...
            ? 0
            : 1;
          const otherTokenInLP: Token =
            underlyingLPDetails.tokenReserves[1 ^ testInutTokenIdxInLP].token;

          const otherTokenInLPAmount: TokenAmount = new TokenAmount(
            otherTokenInLP,
            calcOtherTokenAmount(
              BN.from(
                underlyingLPDetails.tokenReserves[
                  testInutTokenIdxInLP
                ].rawAmount()
              ),
              BN.from(
                underlyingLPDetails.tokenReserves[
                  1 ^ testInutTokenIdxInLP
                ].rawAmount()
              ),
              inAmount
            ).toString()
        ...
        case forgeIdsInBytes.BENQI:
          const underlyingAddress: string = this.yieldContract.underlyingAsset
            .address;
          const exchangeRate: BN = await pendleForgeContract
            .connect(signer.provider)
            .callStatic.getExchangeRate(underlyingAddress, {
        ...
        default:
          throw new Error(
            `Unsupported forge: ${this.yieldContract.forgeId} in OneClickWrapper`
          );
      }
      ...
      if (action == Action.stakeOT || action == Action.stakeOTYT) {
        ...
        const otReserve: BN = BN.from(
          otMarketDetail.tokenReserves[otTokenIdxInMarket].rawAmount()
        );
        const baseTokenReserve: BN = BN.from(
          otMarketDetail.tokenReserves[1 ^ otTokenIdxInMarket].rawAmount()
        );
        ...
        transactions.push({
          ...
          maxPaid: [
            ...
            new TokenAmount(
              ...
              calcSlippedUpAmount(
                BN.from(baseTokenInOtMarketAmount.rawAmount()),
                slippage
              ).toString()
            ),
          ],
      ...
      if (action == Action.stakeYT || action == Action.stakeOTYT) {
        ...
        const baseTokenInYtMarketAmount: TokenAmount =
          addDualDetails.otherTokenAmount;
        ...
        transactions.push({
          ...
          maxPaid: [
            ...
            new TokenAmount(
              ...
              calcSlippedUpAmount(
                BN.from(baseTokenInYtMarketAmount.rawAmount()),
                slippage
              ).toString()
            ),
          ],
      ...
      transactions.forEach((txn: Transaction) => {
        txn.maxPaid.forEach((t: TokenAmount) => {
          if (tokenMap.has(t.token.address)) {
            tokenMap.set(
              t.token.address,
              tokenMap.get(t.token.address)!.add(t.rawAmount())
            );
          } else {
      ...
      transactions.forEach((txn: Transaction) => {
        txn.received.forEach((t: TokenAmount) => {
          if (tokenMap.has(t.token.address)) {
            tokenMap.set(
              t.token.address,
              tokenMap.get(t.token.address)!.sub(t.rawAmount())
            );
          }
        });
      });
      ...
      tokenMap.forEach((amount: BN, t: string) => {
        if (amount.gt(0)) {
          finalTokenAmounts.push(
            new TokenAmount(
              new Token(t, networkInfo.decimalsRecord[t]),
              amount.toString()
            )
          );
        }
    ...
    const simulate = async (
      ...
      const testInputTokenAmount: TokenAmount = testSimulationResult.tokenAmounts.find(
        (t: TokenAmount) =>
    ...
    const send = async (
      ...
      const maxEthPaid: BN = sTxns.reduce((p: BN, txn: Transaction): BN => {
        const ethInThisTxn: BN = txn.maxPaid.reduce(
          (pp: BN, t: TokenAmount): BN => {
      ...
      const currentTime: BN = BN.from(
        await getCurrentTimestamp(signer.provider)
      );
      ...
      if (isUnderlyingLP()) {
        ...
        const preMintTxn: Transaction = sTxns.find(
          (txn: Transaction) => txn.action == TransactionAction.preMint
        )!;
      ...
      if (action == Action.stakeOT || action == Action.stakeOTYT) {
        ...
        dataAddLiqOT.amountTokenDesired = otAddLiqTxn.maxPaid
          .find((t: TokenAmount) =>
            isSameAddress(t.token.address, baseToken.address)
          )!
          .rawAmount();
    ...
    const getOtRewards = async (
      ...
      if (this.yieldContract.forgeIdInBytes == forgeIdsInBytes.JOE_COMPLEX) {
        ...
        const rawAprs: AprInfo[] = await MasterChef.methods(
          signer,
          chainId
        ).getRewardsAprs(pid);
        const principalValuation: CurrencyAmount = await fetchValuation(
          underlyingAmount,
          signer,
          chainId
        );
    ...
    const getAllRewardsFromTxns = async (
      ...
      var otRewards: AprWithPrincipal[] = [],
        ytRewards: AprWithPrincipal[] = [];
      if (action == Action.stakeOT || action == Action.stakeOTYT) {
        const otLpAddLiqTxn: Transaction = getAddOtLiqTxn(
          testSimulation.transactions,
          pendleFixture
        )!;
        otPromises.push(
          getLmRewards(pendleFixture.otStakingPool, otLpAddLiqTxn.received[0])
        );
        otLpValuation = await fetchValuation(
          otLpAddLiqTxn.received[0],
          signer,
          chainId
        );
        otLpSwapFeeApr = await pendleFixture.otMarket
          .methods(signer, chainId)
          .getSwapFeeApr();
        otRewards.push({
          apr: {
      ...
      if (action == Action.stakeYT || action == Action.stakeOTYT) {
        const ytLpAddLiqTxn: Transaction = getAddYtLiqTxn(
          testSimulation.transactions,
          pendleFixture
        )!;
        ytPromises.push(
          getLmRewards(pendleFixture.ytStakingPool, ytLpAddLiqTxn.received[0])
        );
        ytLpValuation = await fetchValuation(
          ytLpAddLiqTxn.received[0],
          signer,
          chainId
        );
        ytLpSwapFeeApr = await pendleFixture.ytMarket
          .methods(signer, chainId)
          .getSwapFeeApr();
        ytRewards.push({
          apr: {
    ...
    const apr = async (action: Action): Promise<WrapperAPRInfo> => {
      ...
      const testSimulation: SimulationDetails = await simulate(
        action,
        inputTokenAmount,
        0
      );
      ...
      for (const t of testSimulation.tokenAmounts) {
        totalPrincipalValuation = totalPrincipalValuation.plus(
          (await fetchValuation(t, signer, chainId)).amount
        );
      }
      // console.log(totalPrincipalValuation.toFixed(DecimalsPrecision));

      var rewardsInfo: rewardsInfo = await getAllRewardsFromTxns(
        action,
        testSimulation,
        pendleFixture
      );
      // console.log("rewardsInfo", JSON.stringify(rewardsInfo, null, '  '));
      const adjustedOTRewards: AprInfo[] = rewardsInfo.otRewards.map(
        (aprWP: AprWithPrincipal) => {
          return {
      ...
      const adjustedYTRewards: AprInfo[] = rewardsInfo.ytRewards.map(
        (aprWP: AprWithPrincipal) => {
          return {
            origin: aprWP.apr.origin,
            apr: new BigNumber(aprWP.apr.apr)
      ...
      return {
        ...
        composition: {
          otPoolApr: adjustedOTRewards.filter((apr: AprInfo) =>
            new BigNumber(apr.apr).gt(0)
          ),
          ytPoolApr: adjustedYTRewards.filter((apr: AprInfo) =>
            new BigNumber(apr.apr).gt(0)
          ),
        },
      };
