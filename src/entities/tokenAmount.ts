...
import { dummyToken, Token } from './token';
...
export class TokenAmount {
  ...
  public constructor(token: Token, amount: string, isRaw: boolean = true) {
    if (isRaw) {
      ...
      this.rawAmnt = new BigNumberjs(amount).times(decimalFactor(token.decimals)).toFixed(0);
    }
    this.token = token;
  }

  public formattedAmount(): string {
    return new BigNumberjs(this.rawAmnt).div(decimalFactor(this.token.decimals)).toString();
  }
  ...
  public static methods({ signer, provider, chainId }: ChainSpecifics): Record<string, any> {
    ...
    const balancesOf = async ({ user, tokens }: { user: string; tokens: Token[] }): Promise<TokenAmount[]> => {
      const multiCallV2Contract: Contract = new Contract(
      ...
      await Promise.all(
        indexRange(0, tokens.length).map(
      ...
      const returnedData: Result_MultiCall[] = (
        await multiCallV2Contract.callStatic.tryBlockAndAggregate(true, calls, HG)
      ).returnData;
      ...
      indexRange(0, tokens.length).forEach((i: number) => {
        ...
        balances[i] = new TokenAmount(tokens[i], balance.toString());
      });
      return balances;
    };
    return {
...
export const dummyTokenAmount: TokenAmount = new TokenAmount(dummyToken, '1000000000000000');
