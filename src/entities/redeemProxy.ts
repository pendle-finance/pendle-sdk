...
export class RedeemProxy {
  public static methods({ signer, provider, chainId }: ChainSpecifics): Record<string, any> {
    ...
    const constructArgsForClaimYieldsMulti = (
      ...
      const lmV1RedeemRequestsForInterests: LmRedeemRequest[] = indexRange(0, lmV1sForInterest.length).map(
        (i: number) => {
          return {
      ...
      const lmV2sForInterest: StakingPool[] = interestStakingPools.filter((s: StakingPool) => s.isLmV2());
      const lmV2sForRewards: StakingPool[] = rewardStakingPools.filter((s: StakingPool) => s.isLmV2());
      const lmV2RedeemRequestsForInterests: LmRedeemRequest[] = indexRange(0, lmV2sForInterest.length).map(
        (i: number) => {
          return {
    ...
    const callStatic: Record<string, any> = {
      redeemMarkets: async (marketsAddresses: string[], userAddress: string): Promise<TokenAmount[]> => {
        switch (proxyVersion) {
          case ProxyVersion.OldSingle: {
            const userInterests: BN[] = await redeemProxyContract.callStatic.redeemMarkets(marketsAddresses, {
              from: userAddress,
            });
            return indexRange(0, marketsAddresses.length).map((i: number) => {
              const marketInfo: PENDLEMARKETNFO = networkInfo.contractAddresses.pendleMarkets.find(
                (m: PENDLEMARKETNFO) => isSameAddress(m.address, marketsAddresses[i])
              )!;
        ...
        switch (proxyVersion) {
          case ProxyVersion.OldSingle: {
            const userRewards: BN[] = (
              await redeemProxyContract.callStatic.redeemOts(otAddresses, { from: userAddress })
            ).map((res: OtRewards) => res.amountRewardOne);
            return indexRange(0, otAddresses.length).map((i: number) => {
              const otInfo: OTINFO = networkInfo.contractAddresses.OTs.find((ot: OTINFO) =>
                isSameAddress(ot.address, otAddresses[i])
              )!;
          ...
          case ProxyVersion.Multi: {
            const userRewards = await redeemProxyContract.callStatic.redeemOts(otAddresses, userAddress, {
              from: userAddress,
            });
      ...
      redeemXyts: async (ytAddresses: string[], userAddress: string): Promise<TokenAmount[]> => {
        switch (proxyVersion) {
          case ProxyVersion.OldSingle: {
            const userInterests: BN[] = await redeemProxyContract.callStatic.redeemXyts(ytAddresses, {
              from: userAddress,
            });
            return indexRange(0, ytAddresses.length).map((i: number) => {
              const ytInfo: YTINFO = networkInfo.contractAddresses.YTs.find((y: YTINFO) =>
                isSameAddress(y.address, ytAddresses[i])
              )!;
              return new TokenAmount(
        ...
        switch (proxyVersion) {
          case ProxyVersion.OldSingle: {
            const userInterests: BN[] = (
              await redeemProxyContract.callStatic.redeemLmInterests(lmAddresses, expiries, userAddress)
            ).lmInterests;
          ...
          case ProxyVersion.Multi: {
            ...
            const redeemRequests: wrappedRequest[] = indexRange(0, lmAddresses.length).map(
            ...
            const userInterestsWithValidExpiries: PairTokenUints[] = (
              await redeemProxyContract.callStatic.redeemLmV1(
                redeemRequestsWithValidExpiries.map((w: wrappedRequest): LmRedeemRequest => w.request),
                userAddress
              )
      ...
      redeemLmRewards: async (
        ...
        switch (proxyVersion) {
          case ProxyVersion.OldSingle: {
            const userRewards: BN[] = (
              await redeemProxyContract.callStatic.redeemLmRewards(lmAddresses, expiries, userAddress)
            ).lmRewards;
            return indexRange(0, lmAddresses.length).map((i: number) => {
              const LMInfo: LMINFO = networkInfo.contractAddresses.stakingPools.find((lm: LMINFO) => {
                return isSameAddress(lm.address, lmAddresses[i]) && lm.expiry!.eq(expiries[i]);
          ...
          case ProxyVersion.Multi: {
            ...
            const userRewardsWithValidExpiries: PairTokenUints[] = (
              await redeemProxyContract.callStatic.redeemLmV1(
                redeemRequestsWithValidExpiries.map((w: wrappedRequest) => w.request),
                userAddress
              )
      ...
      redeemLmV2Interests: async (lmAddresses: string[], userAddress: string): Promise<PairTokenUints[]> => {
        switch (proxyVersion) {
          ...
          case ProxyVersion.Multi: {
            ...
            return (
              await redeemProxyContract.callStatic.redeemLmV2(redeemRequests, userAddress)
            ).map((res: LmRedeemResult) => PairTokenUints.fromContractPairTokenUints(res.interests));
          }
      ...
      redeemTokenDist: async (tokenAddresses: string[], userAddress: string): Promise<TokenAmount[]> => {
        switch (proxyVersion) {
          ...
          case ProxyVersion.Multi: {
            const redeemResults: BN[] = await redeemProxyContract.callStatic.redeemTokenDist(
              tokenAddresses,
              userAddress
            );
            return indexRange(0, tokenAddresses.length).map(
              (i: number): TokenAmount => {
                return new TokenAmount(
                  new Token(tokenAddresses[i], networkInfo.decimalsRecord[tokenAddresses[i]]),
