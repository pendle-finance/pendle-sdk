...
export class Ot extends Token {
  ...
  public constructor(
    address: string,
    decimals: number,
    yieldBearingAddress: string,
    forgeIdInBytes: string,
    expiry: number,
    rewardTokens: string[],
    priceFeedMarketAddress?: string
  ) {
  ...
  public static find(address: string, chainId?: number): Ot {
    ...
    const otInfo: OTINFO | undefined = networkInfo.contractAddresses.OTs.find(
      (o: OTINFO) => {
    ...
    const otherMarkets: MARKETINFO[] | undefined =
      networkInfo.contractAddresses.otherMarkets;
    var priceFeedMarketAddress = undefined;
    if (otherMarkets !== undefined) {
      const marketInfo: MARKETINFO | undefined = otherMarkets.find(
        (m: MARKETINFO) =>
          isSameAddress(m.pair[0], address) || isSameAddress(m.pair[1], address)
      );
    ...
    return new Ot(
      ...
      0, // expiry not used
      otInfo.rewardTokenAddresses === undefined
        ? []
        : otInfo.rewardTokenAddresses,
  ...
  public static methods(
    ...
    const fetchRewards = async (userAddress: string): Promise<OtReward[]> => {
      const OTs: OTINFO[] = networkInfo.contractAddresses.OTs.filter(
        (OTInfo: OTINFO) => Ot.hasRewardsByForgeId(OTInfo.forgeIdInBytes)
      );
      const userRewards: TrioTokenUints[] = await RedeemProxy.methods(
        signer,
        chainId
      ).callStatic.redeemOts(
        OTs.map((OTInfo: OTINFO) => OTInfo.address),
        userAddress
      );
      const formattedResult: OtReward[] = indexRange(0, OTs.length).map(
        (i: number) => {
          return {
  ...
  private static hasRewardsByForgeId(forgeId: string) {
    return !(
      forgeId == forgeIdsInBytes.SUSHISWAP_SIMPLE ||
      forgeId == forgeIdsInBytes.JOE_SIMPLE
    );
  }
