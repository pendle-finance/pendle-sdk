...
export class Ot extends Token {
  ...
  public static find(address: string, chainId?: number): Ot {
    ...
    if (otherMarkets !== undefined) {
      const marketInfo: MARKETINFO | undefined = otherMarkets.find(
        (m: MARKETINFO) => isSameAddress(m.pair[0], address) || isSameAddress(m.pair[1], address)
      );
  ...
  public static methods({ signer, provider, chainId }: ChainSpecifics): Record<string, any> {
    ...
    const fetchRewards = async (userAddress: string): Promise<OtReward[]> => {
      const OTs: OTINFO[] = networkInfo.contractAddresses.OTs.filter((OTInfo: OTINFO) =>
        Ot.hasRewardsByForgeId(OTInfo.forgeIdInBytes)
      );
  ...
  public yieldContract(chainId?: number): YieldContract {
    ...
    return new YieldContract(
      ...
      new Token(this.underlyingAssetAddress, networkInfo.decimalsRecord[this.underlyingAssetAddress]),
      this.expiry!,
      chainId
    );
  }
