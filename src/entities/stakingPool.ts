...
const populatePoolYields = (
  LmInfo: LMINFO,
  interestAmounts: TokenAmount[],
  rewardAmounts: TokenAmount[],
  decimalsRecord: Record<string, number>
): PoolYields => {
  const yields: YieldInfo[] = interestAmounts
    .map(
      (t: TokenAmount): YieldInfo => {
        return {
          yield: t,
...
const populatePoolAccruingRewards = (
  LmInfo: LMINFO,
  tentativeRewards: TokenAmount[],
  currentEpochId: number,
  vestingEpoches: number,
  decimalsRecord: Record<string, number>
): PoolAccruingRewards => {
  return {
    address: LmInfo.address,
    ...
    accruingRewards: indexRange(0, vestingEpoches).map(
      (i: number): FutureEpochRewards => {
        return {
          epochId: i + currentEpochId,
          rewards: tentativeRewards.map(
            (t: TokenAmount) =>
              new TokenAmount(
                t.token,
                BN.from(t.rawAmount())
...
const populatePoolVestedRewards = (
  LmInfo: LMINFO,
  vestedRewards: TokenAmount[][],
  currentEpochId: number,
  decimalsRecord: Record<string, number>
): PoolVestedRewards => {
  return {
    ...
    vestedRewards: indexRange(0, vestedRewards.length).map(
      (i: number): FutureEpochRewards => {
        return {
          epochId: i + currentEpochId,
...
const STAKINGPOOL_NOT_EXIST = new Error(
  'No Staking pool is found at the given address and input token.'
);
...
export class StakingPool {
  ...
  public static find(
    ...
    const lmInfo:
      | LMINFO
      | undefined = networkInfo.contractAddresses.stakingPools.find(
      (s: LMINFO) => {
        return (
          isSameAddress(s.address, address) &&
          isSameAddress(s.inputTokenAddress, inputTokenAddress)
        );
      }
    );
    if (lmInfo === undefined) {
  ...
  public static methods(
    ...
    const liquidityRewardsReaderABI =
    ...
    const fetchClaimableYields = async (
      ...
      promises.push(
        RedeemProxy.methods(signer, chainId).callStatic.redeemLmInterests(
      ...
      promises.push(
        RedeemProxy.methods(signer, chainId).callStatic.redeemLmRewards(
          Lm1s.map((LmInfo: LMINFO) => LmInfo.address),
          Lm1s.map((LmInfo: LMINFO) => LmInfo.expiry),
          userAddress
        )
      ...
      promises.push(
        RedeemProxy.methods(signer, chainId).callStatic.redeemLmV2Interests(
          Lm2s.map((LmInfo: LMINFO) => LmInfo.address),
          userAddress
        )
      );
      promises.push(
        RedeemProxy.methods(signer, chainId).callStatic.redeemLmV2Rewards(
          Lm2s.map((LmInfo: LMINFO) => LmInfo.address),
          userAddress
        )
      );
      ...
      const Lm1InterestsAndRewards = indexRange(0, Lm1s.length).map(
        (i: number) => {
          return populatePoolYields(
            Lm1s[i],
            userLm1Interests[i].toTokenAmounts(chainId),
            userLm1Rewards[i].toTokenAmounts(chainId),
            decimalsRecord
          );
        }
      );

      const Lm2InterestsAndRewards = indexRange(0, Lm2s.length).map(
        (i: number) => {
          return populatePoolYields(
            Lm2s[i],
            userLm2Interests[i].toTokenAmounts(chainId),
            userLm2Rewards[i].toTokenAmounts(chainId),
            decimalsRecord
          );
        }
      );
    ...
    const fetchAccruingRewards = async (
      ...
      const Lm1AccruingRewardsCalls: Call_MultiCall[] = await Promise.all(
        Lm1s.map(async function(LmInfo: LMINFO) {
          return {
            target: liquidityRewardsProxyContract.address,
            callData: (
              await liquidityRewardsProxyContract.populateTransaction.redeemAndCalculateAccruingV1(
                LmInfo.address,
                LmInfo.expiry,
                userAddress
              )
      ...
      const Lm2AccruingRewardsCalls: Call_MultiCall[] = await Promise.all(
        Lm2s.map(async function(LmInfo: LMINFO) {
          return {
            target: liquidityRewardsProxyContract.address,
            callData: (
              await liquidityRewardsProxyContract.populateTransaction.redeemAndCalculateAccruingV2(
                LmInfo.address,
                userAddress
              )
            ).data!,
      ...
      for (var i: number = 0; true; i++) {
        ...
        returnedData = returnedData.concat(
          (
            await multiCallV2Contract.callStatic.tryBlockAndAggregate(
              false,
              calls.slice(leftInd, rightInd),
              HG
            )
          ).returnData
        );
      ...
      const userLm1AccruingRewards: PairTokenUints[] = indexRange(
        0,
        Lm1s.length
      ).map(
        (i: number): PairTokenUints => {
          if (returnedData[i].success) {
            return PairTokenUints.fromContractPairTokenUints(
              liquidityRewardsProxyContract.interface.decodeFunctionResult(
                'redeemAndCalculateAccruingV1',
                returnedData[i].returnData
      ...
      const userLm2AccruingRewards: PairTokenUints[] = indexRange(
        Lm1s.length,
        Lm1s.length + Lm2s.length
      ).map(
        (i: number): PairTokenUints => {
          if (returnedData[i].success) {
            return PairTokenUints.fromContractPairTokenUints(
      ...
      const currentEpochId = getCurrentEpochId(
        currentTime,
        getLMStartTime(chainId),
        LMEpochDuration
      );

      const userLm1AccruingRewardsFormatted = indexRange(0, Lm1s.length).map(
        (i: number) => {
          return populatePoolAccruingRewards(
            Lm1s[i],
            userLm1AccruingRewards[i].toTokenAmounts(chainId),
            currentEpochId,
            VestingEpoches,
            decimalsRecord
          );
      ...
      const userLm2AccruingRewardsFormatted = indexRange(0, Lm2s.length).map(
        (i: number) => {
          return populatePoolAccruingRewards(
            Lm2s[i],
            userLm2AccruingRewards[i].toTokenAmounts(chainId),
            currentEpochId,
            VestingEpoches,
            decimalsRecord
          );
      ...
      return userLm1AccruingRewardsFormatted.concat(
        userLm2AccruingRewardsFormatted
      );
    };
    ...
    const fetchVestedRewards = async (
      ...
      const Lm1VestedRewardsCalls: Call_MultiCall[] = await Promise.all(
        Lm1s.map(async function(LmInfo: LMINFO) {
          return {
            target: liquidityRewardsProxyContract.address,
            callData: (
              await liquidityRewardsProxyContract.populateTransaction.redeemAndCalculateVestedV1(
                LmInfo.address,
                [LmInfo.expiry],
                userAddress
              )
      ...
      const Lm2VestedRewardsCalls: Call_MultiCall[] = await Promise.all(
        Lm2s.map(async function(LmInfo: LMINFO) {
          return {
            target: liquidityRewardsProxyContract.address,
            callData: (
              await liquidityRewardsProxyContract.populateTransaction.redeemAndCalculateVestedV2(
                LmInfo.address,
                userAddress
              )
            ).data!,
      ...
      const userLm1VestedRewards: PairTokenUints[][] = indexRange(
        0,
        Lm1s.length
      ).map((i: number): PairTokenUints[] => {
        if (returnedData[i].success) {
          return liquidityRewardsProxyContract.interface
            .decodeFunctionResult(
              'redeemAndCalculateVestedV1',
              returnedData[i].returnData
            )
            .vestedRewards.map((pt: any) =>
              PairTokenUints.fromContractPairTokenUints(pt)
            );
        } else {
          return Array(VestingEpoches - 1).fill(PairTokenUints.EMPTY);
        }
      });
      const userLm2VestedRewards: PairTokenUints[][] = indexRange(
        Lm1s.length,
        Lm1s.length + Lm2s.length
      ).map((i: number): PairTokenUints[] => {
        if (returnedData[i].success) {
      ...
      const userLm1VestedRewardsFormatted = indexRange(0, Lm1s.length).map(
        (i: number) => {
          return populatePoolVestedRewards(
            Lm1s[i],
            userLm1VestedRewards[i].map((t: PairTokenUints) =>
              t.toTokenAmounts(chainId)
            ),
            currentEpochId,
            decimalsRecord
          );
      ...
      const userLm2VestedRewardsFormatted = indexRange(0, Lm2s.length).map(
        (i: number) => {
          return populatePoolVestedRewards(
            Lm2s[i],
            userLm2VestedRewards[i].map((t: PairTokenUints) =>
              t.toTokenAmounts(chainId)
            ),
            currentEpochId,
            decimalsRecord
      ...
      return userLm1VestedRewardsFormatted.concat(
        userLm2VestedRewardsFormatted
      );
    };
  ...
  public methods(
    ...
    const networkInfo: NetworkInfo = distributeConstantsByNetwork(chainId);
    const stakingPoolContract: Contract = new Contract(
      this.address,
      getABIByStakingPoolType(this.contractType).abi,
      signer.provider
    );
    let expiry: number = 0,
      market: PendleMarket;
    ...
    const stake = async (
      ...
      if (this.contractType == StakingPoolType.LmV1) {
        const args: any[] = [expiry, BN.from(amount.rawAmount())];
        ...
        const args: any[] = [BN.from(amount.rawAmount())];
    ...
    const unstake = async (
      ...
      if (this.contractType == StakingPoolType.LmV1) {
        ...
        const userPendleBalance: BN = BN.from(
          (await balanceOf(userAddress)).amount.rawAmount()
        );
        const userShareBalance: BN = await stakingPoolContract.balances(
          userAddress
        );
        const shareToRedeem: BN = BN.from(amount.rawAmount())
          .mul(userShareBalance)
          .div(userPendleBalance);

        const args: any[] = [shareToRedeem];
    ...
    const getTotalStaked = async (): Promise<StakedAmount> => {
      if (this.isLmV1()) {
        const totalStakeLP: BN = (
          await stakingPoolContract.readExpiryData(expiry)
        ).totalStakeLP;
        const marketLPPrice: BigNumber = await market
          .methods(signer, chainId)
          .getLPPriceBigNumber();
        return populateStakedAmount(totalStakeLP, marketLPPrice);
      } else if (this.isLmV2()) {
        const totalStakeLP: BN = await stakingPoolContract.totalStake();
        const marketLPPrice: BigNumber = await fetchTokenPrice({
    ...
    const balanceOf = async (address: string): Promise<StakedAmount> => {
      if (this.isLmV1()) {
        const stakedLP: BN = (
          await stakingPoolContract.readUserSpecificExpiryData(expiry, address)
        ).balances;
        const marketLPPrice: BigNumber = await market
          .methods(signer, chainId)
          .getLPPriceBigNumber();
        ...
        const userPendleBalance: BN = await stakingPoolContract.callStatic.leave(
          userShareBalance,
          { from: address }
        );
        const PENDLEPrice: BigNumber = await fetchTokenPrice({
    ...
    const getTotalRewardForOneEpoch = async (
      epochData: any,
      allocPoint: BN = ALLOCATION_DENOMINATOR
    ): Promise<TokenAmount[]> => {
      if (
        this.contractType == StakingPoolType.LmV1 ||
        this.contractType == StakingPoolType.LmV2
      ) {
        ...
        this.contractType == StakingPoolType.LmV1Multi ||
        this.contractType == StakingPoolType.LmV2Multi
      ) {
        const rewardTokens: PairTokens = await stakingPoolContract.rewardTokens();
        const totalRewards: PairUints = epochData.totalRewards;
        const userRewards: PairTokenUints = new PairTokenUints(
          rewardTokens,
          totalRewards
        );
    ...
    const rewardAprs = async (): Promise<AprInfo[]> => {
      if (this.isLmV1()) {
        const stakedUSDValue: BigNumber = new BigNumber(
          (await getTotalStaked()).valuation.amount
        );
        const startTime: BN = await stakingPoolContract.startTime();
        const curTime: number = await getCurrentTimestamp(signer.provider);
        const currentEpochId: number = getCurrentEpochId(
          curTime,
          startTime,
          LMEpochDuration
        );
        const latestSetting: any = await stakingPoolContract.latestSetting();
        const epochData: any = await stakingPoolContract.readEpochData(
          currentEpochId
        );

        const settingId =
          currentEpochId >= latestSetting.firstEpochToApply
            ? latestSetting.id
            : epochData.settingId;

        const allocPoint: BN = await stakingPoolContract.allocationSettings(
          settingId,
          expiry
        );
        const rewardsForThisExpiry: TokenAmount[] = await getTotalRewardForOneEpoch(
          epochData,
          allocPoint
        );

        const rewardsValue: BigNumber[] = await Promise.all(
          rewardsForThisExpiry.map(
            async (t: TokenAmount): Promise<BigNumber> => {
              return await calcValuation(
                ...
                BN.from(t.rawAmount()),
                networkInfo.decimalsRecord[t.token.address]
              );
            }
          )
        );
        const rewardApr: BigNumber = calcLMRewardApr(
          rewardsValue.reduce(
            (p: BigNumber, c: BigNumber) => p.plus(c),
            new BigNumber(0)
          ),
          stakedUSDValue,
          52
        ...
        const rewards: TokenAmount[] = await getTotalRewardForOneEpoch(
          epochData
        );
        const rewardsValue: BigNumber[] = await Promise.all(
          rewards.map(
            async (t: TokenAmount): Promise<BigNumber> => {
              const tokenPrice: BigNumber = await fetchTokenPrice({
                address: t.token.address,
                signer,
                chainId,
              });
              const valuation: BigNumber = await calcValuation(
                tokenPrice,
                BN.from(t.rawAmount()),
                networkInfo.decimalsRecord[t.token.address]
              );
        ...
        if (
          this.interestTokens !== undefined &&
          this.interestTokens?.length > 0
        ) {
          const SLPLiquidityMiningContract: Contract = new Contract(
            this.address,
            contracts.PendleSLPLiquidityMining.abi,
            signer.provider
          );
          const pid: BN = await SLPLiquidityMiningContract.pid();
          aprInfos = aprInfos.concat(
            await MasterChef.methods(signer, chainId).getRewardsAprs(pid)
          );
        }
        return aprInfos;
      } else if (this.contractType == StakingPoolType.PendleSingleSided) {
        const singleStakingManager: Contract = new Contract(
          networkInfo.contractAddresses.misc.PendleSingleStakingManager,
          contracts.PendleSingleStakingManager.abi,
          signer.provider
        );
        ...
        return [
          {
            origin: 'Pendle',
            apr: calcLMRewardApr(
              new BigNumber(rewardPerBlock.mul(blockPerDay).toString()),
              new BigNumber(PendleStaked.toString()),
              365
            ).toFixed(DecimalsPrecision),
          },
        ];
    ...
    const populateStakedAmount = (
      rawAmount: BN,
      price: BigNumber
    ): StakedAmount => {
      return {
        amount: new TokenAmount(this.inputToken, rawAmount.toString()),
        valuation: {
          currency: 'USD',
          amount: calcValuation(
            price,
            rawAmount,
            networkInfo.decimalsRecord[this.inputToken.address]
          ).toFixed(DecimalsPrecision),
        },
      };
    };
