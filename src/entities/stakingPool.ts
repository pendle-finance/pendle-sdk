...
import { ALLOCATION_DENOMINATOR, dummyAddress, HG, LMEpochDuration, ONE_DAY, VestingEpoches } from '../constants';
import { contracts } from '../contracts';
import { Token } from './token';
import { TokenAmount } from './tokenAmount';
import { LMINFO, NetworkInfo, StakingPoolType } from '../networks';
...
import { AprInfo, ChainSpecifics, PairTokens, PairUints } from './types';
...
const populatePoolYields = (
  ...
  return {
    ...
    inputToken: new Token(LmInfo.inputTokenAddress, decimalsRecord[LmInfo.inputTokenAddress]),
    yields: yields,
  };
};
...
export class StakingPool {
  ...
  public static find(address: string, inputTokenAddress: string, chainId?: number): StakingPool {
    ...
    return new StakingPool(
      address.toLowerCase(),
      new Token(inputTokenAddress.toLowerCase(), networkInfo.decimalsRecord[inputTokenAddress.toLowerCase()]),
      lmInfo.rewardTokenAddresses.map((s: string) => {
        return new Token(s.toLowerCase(), networkInfo.decimalsRecord[s.toLowerCase()]);
      }),
      lmInfo.contractType,
  ...
  public static methods({ signer, provider, chainId }: ChainSpecifics): Record<string, any> {
    ...
    const fetchClaimableYields = async (userAddress: string): Promise<PoolYields[]> => {
      const promises: Promise<PairTokenUints[]>[] = [];
      promises.push(
        RedeemProxy.methods({ signer, provider, chainId }).callStatic.redeemLmInterests(
      ...
      promises.push(
        RedeemProxy.methods({ signer, provider, chainId }).callStatic.redeemLmRewards(
          Lm1s.map((LmInfo: LMINFO) => LmInfo.address),
          Lm1s.map((LmInfo: LMINFO) => LmInfo.expiry),
          userAddress
        )
      ...
      promises.push(
        RedeemProxy.methods({ signer, provider, chainId }).callStatic.redeemLmV2Interests(
          Lm2s.map((LmInfo: LMINFO) => LmInfo.address),
          userAddress
        )
      );
      promises.push(
        RedeemProxy.methods({ signer, provider, chainId }).callStatic.redeemLmV2Rewards(
          Lm2s.map((LmInfo: LMINFO) => LmInfo.address),
          userAddress
        )
      );
    ...
    const fetchAccruingRewards = async (userAddress: string): Promise<PoolAccruingRewards[]> => {
      ...
      for (var i: number = 0; true; i++) {
        ...
        returnedData = returnedData.concat(
          (await multiCallV2Contract.callStatic.tryBlockAndAggregate(false, calls.slice(leftInd, rightInd), HG))
            .returnData
        );
        if (rightInd == calls.length) break;
      }

      const userLm1AccruingRewards: PairTokenUints[] = indexRange(0, Lm1s.length).map(
      ...
      const userLm2AccruingRewards: PairTokenUints[] = indexRange(Lm1s.length, Lm1s.length + Lm2s.length).map(
        (i: number): PairTokenUints => {
          if (returnedData[i].success) {
            return PairTokenUints.fromContractPairTokenUints(
              liquidityRewardsProxyContract.interface.decodeFunctionResult(
    ...
    const fetchVestedRewards = async (userAddress: string): Promise<PoolVestedRewards[]> => {
      ...
      const userLm1VestedRewards: PairTokenUints[][] = indexRange(0, Lm1s.length).map((i: number): PairTokenUints[] => {
        if (returnedData[i].success) {
          return liquidityRewardsProxyContract.interface
            .decodeFunctionResult('redeemAndCalculateVestedV1', returnedData[i].returnData)
            .vestedRewards.map((pt: any) => PairTokenUints.fromContractPairTokenUints(pt));
        } else {
          return Array(VestingEpoches - 1).fill(PairTokenUints.EMPTY);
        }
      });
      const userLm2VestedRewards: PairTokenUints[][] = indexRange(Lm1s.length, Lm1s.length + Lm2s.length).map(
        (i: number): PairTokenUints[] => {
          if (returnedData[i].success) {
            return liquidityRewardsProxyContract.interface
              .decodeFunctionResult('redeemAndCalculateVestedV2', returnedData[i].returnData)
              .vestedRewards.map((pt: any) => PairTokenUints.fromContractPairTokenUints(pt));
          } else {
            return Array(VestingEpoches - 1).fill(PairTokenUints.EMPTY);
          }
      ...
      const userLm1VestedRewardsFormatted = indexRange(0, Lm1s.length).map((i: number) => {
        return populatePoolVestedRewards(
          Lm1s[i],
          userLm1VestedRewards[i].map((t: PairTokenUints) => t.toTokenAmounts(chainId)),
          currentEpochId,
          decimalsRecord
        );
      });
      const userLm2VestedRewardsFormatted = indexRange(0, Lm2s.length).map((i: number) => {
        return populatePoolVestedRewards(
          Lm2s[i],
          userLm2VestedRewards[i].map((t: PairTokenUints) => t.toTokenAmounts(chainId)),
          currentEpochId,
          decimalsRecord
        );
      });
  ...
  public methods({ signer, provider, chainId }: ChainSpecifics): Record<string, any> {
    ...
    const stake = async (amount: TokenAmount): Promise<providers.TransactionResponse> => {
      if (this.contractType == StakingPoolType.LmV1) {
        ...
        const args: any[] = [await signer!.getAddress(), BN.from(amount.rawAmount())];
        return submitTransaction(stakingPoolContract, signer!, 'stake', args);
        ...
        const args: any[] = [await signer!.getAddress(), expiry, BN.from(amount.rawAmount())];
    ...
    const getTotalStaked = async (): Promise<StakedAmount> => {
      if (this.isLmV1()) {
        ...
        const marketLPPrice: BigNumber = await market.methods({ signer, provider, chainId }).getLPPriceBigNumber();
        return populateStakedAmount(totalStakeLP, marketLPPrice);
      } else if (this.isLmV2()) {
        const totalStakeLP: BN = await stakingPoolContract.totalStake();
        const marketLPPrice: BigNumber = await fetchTokenPrice({ address: this.inputToken.address, provider, chainId });
        return populateStakedAmount(totalStakeLP, marketLPPrice);
      } else if (this.contractType == StakingPoolType.PendleSingleSided) {
        const PENDLEContract: Contract = new Contract(this.inputToken.address, contracts.IERC20.abi, provider);
        const totalStake: BN = await PENDLEContract.balanceOf(this.address);
        const PENDLEPrice: BigNumber = await fetchTokenPrice({ address: this.inputToken.address, provider, chainId });
    ...
    const getTotalRewardForOneEpoch = async (
      ...
      if (this.contractType == StakingPoolType.LmV1 || this.contractType == StakingPoolType.LmV2) {
        ...
        return [new TokenAmount(rewardToken, epochData.totalRewards.mul(allocPoint).div(ALLOCATION_DENOMINATOR))];
    ...
    const rewardAprs = async (): Promise<AprInfo[]> => {
      if (this.isLmV1()) {
        ...
        const curTime: number = await getCurrentTimestamp(provider);
        ...
        const settingId = currentEpochId >= latestSetting.firstEpochToApply ? latestSetting.id : epochData.settingId;
          ...
          rewardsForThisExpiry.map(
            async (t: TokenAmount): Promise<BigNumber> => {
              return await calcValuation(
                await fetchTokenPrice({ address: t.token.address, provider, chainId }),
                BN.from(t.rawAmount()),
        ...
        const rewardApr: BigNumber = calcLMRewardApr(
          rewardsValue.reduce((p: BigNumber, c: BigNumber) => p.plus(c), new BigNumber(0)),
          stakedUSDValue,
          52
        );
        return [
        ...
        const rewardsValue: BigNumber[] = await Promise.all(
          rewards.map(
            async (t: TokenAmount): Promise<BigNumber> => {
              const tokenPrice: BigNumber = await fetchTokenPrice({ address: t.token.address, provider, chainId });
        ...
        if (this.interestTokens !== undefined && this.interestTokens?.length > 0) {
          ...
          aprInfos = aprInfos.concat(await MasterChef.methods({ signer, provider, chainId }).getRewardsAprs(pid));
        }
        return aprInfos;
      } else if (this.contractType == StakingPoolType.PendleSingleSided) {
    ...
    const populateStakedAmount = (rawAmount: BN, price: BigNumber): StakedAmount => {
      return {
        ...
        valuation: {
          currency: 'USD',
          amount: calcValuation(price, rawAmount, networkInfo.decimalsRecord[this.inputToken.address]).toFixed(
            DecimalsPrecision
          ),
        },
      };
