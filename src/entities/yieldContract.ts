...
export class YieldContract {
  ...
  public methods(
    ...
    if (
      networkInfo.contractAddresses.forges[this.forgeIdInBytes] === undefined
    ) {
      return Error(
        `No such forge with forgeId ${this.forgeIdInBytes} in this network.`
      );
    }
    const forgeAddress =
      networkInfo.contractAddresses.forges[this.forgeIdInBytes];
    const pendleForgeContract = new Contract(
      forgeAddress,
      getABIByForgeId(this.forgeIdInBytes).abi,
      signer.provider
    );
    ...
    const useCompoundMath = (): boolean => {
      return (
        this.forgeIdInBytes == forgeIdsInBytes.COMPOUND_UPGRADED ||
    ...
    const mintDetails = async (toMint: TokenAmount): Promise<TokenAmount[]> => {
      if (
        this.forgeIdInBytes == forgeIdsInBytes.AAVE ||
        this.forgeIdInBytes == forgeIdsInBytes.COMPOUND
      ) {
        const response = await pendleForgeContract
          .connect(signer.provider)
          .callStatic.mintOtAndXyt(
        ...
        const exchangeRate: BN = await pendleForgeContract
          .connect(signer.provider)
          .callStatic.getExchangeRate(this.underlyingAsset.address, {
            from: networkInfo.contractAddresses.misc.PendleRouter,
          });
        const ot: string = (
          await pendleDataContract.callStatic.otTokens(
            this.forgeIdInBytes,
            this.underlyingAsset.address,
            this.expiry
          )
        ...
        const yt: string = (
          await pendleDataContract.callStatic.xytTokens(
            this.forgeIdInBytes,
            this.underlyingAsset.address,
            this.expiry
          )
        ).toLowerCase();
        const amountToMint: BN = useCompoundMath()
          ? cmul(BN.from(toMint.rawAmount()), exchangeRate)
          : rmul(BN.from(toMint.rawAmount()), exchangeRate);
        return [
          new TokenAmount(
            new Token(ot, networkInfo.decimalsRecord[yt], this.expiry),
            amountToMint.toString()
          ),
          new TokenAmount(
            new Token(yt, networkInfo.decimalsRecord[yt], this.expiry),
            amountToMint.toString()
          ),
        ];
      }
    ...
    const redeemDetails = async (
      amountToRedeem: TokenAmount,
      userAddress: string
    ): Promise<RedeemDetails> => {
      const interestRedeemed: BN = await pendleForgeContract
        .connect(signer.provider)
      ...
      const testMintedAmount: TokenAmount[] = await mintDetails(
        new TokenAmount(yToken, testMintAmount.toString())
      );
      ...
      switch (this.forgeIdInBytes) {
        ...
        case forgeIdsInBytes.COMPOUND:
          return new TokenAmount(
            this.underlyingAsset,
            (
              await pendleForgeContract.initialRate(
                this.underlyingAsset.address
              )
            )
              .div(decimalFactor(10))
              .toString()
          );
        ...
        case forgeIdsInBytes.JOE_COMPLEX:
          return new TokenAmount(
            ...
            calcPrincipalForSLPYT(
              await pendleForgeContract.callStatic.getExchangeRate(
                this.underlyingAsset.address
              ),
              networkInfo.decimalsRecord[this.underlyingAsset.address]
            ).toString()
        ...
        default:
          throw Error(
            `Unsupported forgeId in getPrincipalPerYT ${this.forgeId}`
          );
      }
    };
