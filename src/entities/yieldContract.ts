...
export class YieldContract {
  ...
  public methods(
    ...
    if (networkInfo.contractAddresses.forges[this.forgeId] === undefined) {
      return Error(
        `No such forge with forgeId ${this.forgeId} in this network.`
      );
    }
    ...
    const PendleForgeContract = new Contract(
      forgeAddress,
      getABIByForgeId(this.forgeId).abi,
      signer.provider
    );
    ...
    const mintDetails = async (toMint: TokenAmount): Promise<TokenAmount[]> => {
      if (this.forgeId !== forgeIds.SUSHISWAP_COMPLEX) {
        const response = await PendleForgeContract.connect(
          signer.provider
        ...
        const exchangeRate: BN = await PendleForgeContract.connect(
          signer.provider
    ...
    const mint = async (
      ...
      return PendleRouterContract.connect(signer).tokenizeYield(
        this.forgeId,
        this.underlyingAsset.address,
        this.expiry,
        toMint.rawAmount(),
    ...
    const redeemDetails = async (
      ...
      const interestRedeemed: BN = await PendleForgeContract.connect(
        signer.provider
      ...
      switch (this.forgeId) {
        ...
        case forgeIds.COMPOUND: {
          const initialRate: BN =
            await PendleForgeContract.callStatic.initialRate(
              this.underlyingAsset.address
            );
          const currentRate: BN =
