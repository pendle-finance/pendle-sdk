...
import { ETHAddress, forgeIdsInBytes, ONE_DAY, ZERO } from '../constants';
import {
  fetchAaveYield,
  fetchBenqiYield,
  fetchCompoundYield,
...
import { fetchTokenPrice, fetchValuation } from '../fetchers';
...
export class Market {
  ...
  public constructor(
    ...
    tokens: Token[],
    protocol: MarketProtocols
  ) {
    this.address = marketAddress;
    this.tokens = tokens.map(
      (t: Token) => new Token(t.address.toLowerCase(), t.decimals, t.expiry)
    );
  ...
  public static find(address: string, chainId?: number): Market {
    try {
      ...
      const marketInfo:
        ...
        (m: MARKETINFO) => isSameAddress(address, m.address)
      );
      if (
        marketInfo !== undefined &&
        (marketInfo.platform == MarketProtocols.Sushiswap ||
          marketInfo.platform == MarketProtocols.TraderJoe)
      ) {
        return new UniForkMarket(
          address,
          marketInfo.pair.map(
            (ad: string) => new Token(ad, networkInfo.decimalsRecord[ad])
          )
        );
      } else {
...
export class PendleMarket extends Market {
  ...
  public static find(address: string, chainId?: number): PendleMarket {
    ...
    const marketInfo:
      | PENDLEMARKETNFO
      | undefined = networkInfo.contractAddresses.pendleMarkets.find(
      (m: PENDLEMARKETNFO) => {
  ...
  public yieldContract(chainId?: number): YieldContract {
    const networkInfo: NetworkInfo = distributeConstantsByNetwork(chainId);
    const ytInfo: YTINFO[] = networkInfo.contractAddresses.YTs.filter(
      (yt: YTINFO) => isSameAddress(yt.address, this.tokens[0].address)
    );
    if (ytInfo.length === 0) {
      throw Error(
        `YT with address ${this.tokens[0].address} not found on this network`
      );
    }
  ...
  public static methods(
    ...
    const networkInfo: NetworkInfo = distributeConstantsByNetwork(chainId);
    const markets: PENDLEMARKETNFO[] =
      networkInfo.contractAddresses.pendleMarkets;
    ...
    const fetchInterests = async (
      ...
      const formattedResult: YtOrMarketInterest[] = indexRange(
        0,
        markets.length
      ).map((i: number) => {
        return {
    ...
    const getLiquidityTransactions = (
      query: PendleAmmQuery
    ): Promise<TRANSACTION[]> => {
  ...
  public methods(
    ...
    const transactionFetcher: TransactionFetcher = new TransactionFetcher(
      chainId === undefined ? 1 : chainId
    );
    const underlyingAsset: string = networkInfo.contractAddresses.YTs.find(
      (ytInfo: YTINFO) => isSameAddress(ytInfo.address, this.tokens[0].address)
    )!.underlyingAssetAddress;
    const yieldContract: YieldContract = this.yieldContract(chainId);
    const forgeAddress =
      networkInfo.contractAddresses.forges[yieldContract.forgeIdInBytes];
    const pendleForgeContract = new Contract(
      forgeAddress,
      getABIByForgeId(yieldContract.forgeIdInBytes).abi,
      signer.provider
    );
      ...
      try {
        switch (yieldContract.forgeIdInBytes) {
          case forgeIdsInBytes.AAVE:
            underlyingYieldRate = await fetchAaveYield(
              yieldContract.underlyingAsset.address
            );
            break;
          ...
          case forgeIdsInBytes.JOE_SIMPLE:
            underlyingYieldRate = await fetchSushiForkYield(
              yieldBearingAsset,
              chainId
            );
            break;
    ...
    const readMarketDetails = async (): Promise<MarketDetails> => {
      ...
      const yieldBearingAsset: string = Yt.find(this.tokens[0].address, chainId)
        .yieldBearingAddress;
      ...
      promises.push(
        getVolume(currentTime - ONE_DAY.toNumber(), currentTime).then(
          (res: CurrencyAmount) => (volumeToday = res)
        )
      );
      promises.push(
        getVolume(
          currentTime - ONE_DAY.mul(2).toNumber(),
          currentTime - ONE_DAY.toNumber()
        ).then((res: CurrencyAmount) => (volumeYesterday = res))
      );

      var liquidityToday: CurrencyAmount = ZeroCurrencyAmount;
      promises.push(
        getLiquidityValue(marketReserves).then(
          (res: CurrencyAmount) => (liquidityToday = res)
        )
      );
      var liquidityYesterday: CurrencyAmount = ZeroCurrencyAmount;
      promises.push(
        getPastLiquidityValue().then(
          (res: CurrencyAmount) => (liquidityYesterday = res)
        )
      );
      ...
      } = await getYTPriceAndImpliedYield(marketReserves);

      const volume24hChange: string = new BigNumber(volumeYesterday.amount).lte(
        0
      )
      ...
      const liquidity24HChange: string = new BigNumber(
        liquidityYesterday.amount
      ).lte(0)
    ...
    const swapExactInDetails = async (
      ...
      const priceImpact: BigNumber = calcPriceImpact(
        currentRateWithSwapFee,
        avgRate
      );

      if (
        isSameAddress(
          tokenDetailsRelative.outToken.address,
    ...
    const swapExactOutDetails = async (
      ...
      if (
        isSameAddress(
          tokenDetailsRelative.inToken.address,
          networkInfo.contractAddresses.tokens.WETH
        )
      ) {
    ...
    const swapExactIn = async (
      ...
      const details: SwapDetails = await swapExactInDetails(
        inTokenAmount,
        slippage
      );
      const args: any[] = [
      ...
      const ETHAmountToSend: BN = isSameAddress(
        details.inAmount.token.address,
        ETHAddress
      )
    ...
    const swapExactOut = async (
      ...
      const details: SwapDetails = await swapExactOutDetails(
        outTokenAmount,
        slippage
      );
      const args: any[] = [
    ...
    const addDualDetails = async (
      ...
      const shareOfPool: BigNumber = calcShareOfPool(
        tokenDetailsRelative.inReserve,
        inAmount
      );

      const totalSupply: BN = await marketContract.totalSupply();
      const lpMinted: BN = totalSupply
        .mul(inAmount)
        .div(tokenDetailsRelative.inReserve);
    ...
    const addDual = async (
      ...
      const xytTokenAmount: TokenAmount = isSameAddress(
        tokenAmounts[0].token.address,
        this.tokens[0].address
      )
        ? tokenAmounts[0]
        : tokenAmounts[1];
      const baseTokenAmount: TokenAmount = isSameAddress(
        tokenAmounts[0].token.address,
        this.tokens[0].address
      )
        ? tokenAmounts[1]
      ...
      const xytMinAmount: BN = calcSlippedDownAmount(
        desiredXytAmount,
        slippage
      );
      const tokenMinAmount: BN = calcSlippedDownAmount(
        desiredTokenAmount,
        slippage
      );
      const args: any[] = [
    ...
    const addSingle = async (
      ...
      const isForXyt: boolean = isSameAddress(
      ...
      const ETHAmountToSend: BN =
        !isForXyt && isSameAddress(tokenAmount.token.address, ETHAddress)
    ...
    const getLpAmountByFraction = async (percentage: number): Promise<BN> => {
      ...
      const userLpBalance: BN = await marketContract.balanceOf(
    ...
    const removeDualDetails = async (
      ...
      const outBaseToken: Token = isSameAddress(
      ...
      if (redeemAmount.eq(0)) {
        return {
          tokenAmounts: [
            new TokenAmount(this.tokens[0], '0'),
            new TokenAmount(outBaseToken, '0'),
          ],
        };
      }
      ...
      const totalSupplyLp: BN = await marketContract.totalSupply();
      const marketReserves: MarketReservesRaw = await marketContract.getReserves();
      const xytRedeemAmout: BN = marketReserves.xytBalance
        .mul(redeemAmount)
        .div(totalSupplyLp);
      const baseTokenRedeemAmount: BN = marketReserves.tokenBalance
        .mul(redeemAmount)
        .div(totalSupplyLp);
      return {
        tokenAmounts: [
    ...
    const removeDual = async (
      ...
      const details: RemoveDualLiquidityDetails = await removeDualDetails(
        percentage,
        slippage
      );
    ...
    const removeSingle = async (
      ...
      const details: RemoveSingleLiquidityDetails = await removeSingleDetails(
        percentage,
        outToken,
        slippage
      );
    ...
    const getVolume = async (
      ...
      while (f) {
        ...
        for (let i: number = 0; i < result.length; i++) {
          if (
            result[i].timestamp! > leftBound &&
            result[i].timestamp! <= rightBound
          ) {
    ...
    const getLiquidityValueBigNumber = async (
      marketReserve: MarketReservesRaw
    ): Promise<BigNumber> => {
      const baseTokenPrice: BigNumber = await fetchTokenPrice({
        address: this.tokens[1].address,
        signer,
        chainId,
      });
    ...
    const getLiquidityValue = async (
      marketReserve: MarketReservesRaw
    ): Promise<CurrencyAmount> => {
      const totalLiquidityUSD: BigNumber = await getLiquidityValueBigNumber(
        marketReserve
      );
      return {
        currency: 'USD',
    ...
    const getYTPrice = async (
      marketReserves: MarketReservesRaw | undefined
    ): Promise<BigNumber> => {
    ...
    const getYTPriceAndImpliedYield = async (
      ...
      const principalPerYT: TokenAmount = await yieldContract
        ...
        .getPrincipalPerYT();
      const p: BigNumber = ytPrice.dividedBy(
        (await fetchValuation(principalPerYT, signer, chainId)).amount
      );
      ...
      const currentTime: number = await getCurrentTimestamp(signer.provider);
      const daysLeft: BigNumber = new BigNumber(
        yieldContract.expiry - currentTime
      ).dividedBy(24 * 3600);
    ...
    const getLPPriceBigNumber = async (): Promise<BigNumber> => {
      const marketReserves: MarketReservesRaw = await marketContract.getReserves();
      const liquidity: BigNumber = await getLiquidityValueBigNumber(
        marketReserves
      );
      const totalSupplyLp: BN = await marketContract.totalSupply();
    ...
    const getSwapFeeAprBigNumber = async (): Promise<BigNumber> => {
      ...
      const volumeToday: CurrencyAmount = await getVolume(
        currentTime - ONE_DAY.toNumber(),
        currentTime
      );

      var liquidityToday: CurrencyAmount = await getLiquidityValue(
        marketReserves
      );
      const swapFee: BN = await pendleDataContract.swapFee();
      const protocolFee: BN = await pendleDataContract.protocolSwapFee();
...
export class UniForkMarket extends Market {
    ...
    const fetchClaimableRewardsFromOTMarkets = async (
      ...
      const rawResult: TokenAmount[][] = await Promise.all(
        markets.map(async (m: UniForkMarket) => {
          return m
            .methods(signer, chainId)
            .fetchRewardsFromOtReserves(userAddress);
        })
      );
  ...
  public methods(
    ...
    const readMarketDetails = async (): Promise<OtherMarketDetails> => {
      const marketContract: Contract = new Contract(
        this.address,
      ...
      const singleSidedLiquidity: CurrencyAmount = await fetchValuation(
        tokenAmount0,
        signer,
        chainId
      );
      const liquidity: CurrencyAmount = {
        ...
        amount: new BigNumber(singleSidedLiquidity.amount)
          .multipliedBy(2)
          .toFixed(DecimalsPrecision),
      };
      return {
    ...
    const fetchRewardsFromOtReserves = async (
      userAddress: string
    ): Promise<TokenAmount[]> => {
      var res: TokenAmount[] = [];
      for (const t of this.tokens) {
        try {
          ...
          res = res.concat(
            redeemResults.filter((tokenAmount: TokenAmount) =>
              BN.from(tokenAmount.rawAmount()).gt(0)
            )
          );
        } catch (err) {}
      }
