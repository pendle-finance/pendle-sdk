...
async function getSushiForkPairInfo(pairAddress: string, chainId: number | undefined): Promise<any> {
  if (chainId === undefined || chainId == 1) {
    return (await request(sushiswapSubgraphApi, sushiswapReservesQuery(pairAddress))).pairs[0];
  } else if (chainId == 43114) {
    return (await request(traderJoeSubgraphApi, sushiswapReservesQuery(pairAddress))).pairs[0];
  } else {
...
export async function fetchCTokenPrice(
  ...
  const adjustedExchangeRate: BigNumber = new BigNumber(exchangeRate.toString()).div(
    decimalFactor(10 + networkInfo.decimalsRecord[underlyingAsset.toLowerCase()])
  );
...
export async function fetchxJOEPrice(
  ...
  return (await fetchTokenPrice({ address: JOEAddress, provider, chainId })).multipliedBy(
    await getxJOEExchangeRate(xJOEAddress, JOEAddress, provider)
  );
}
...
export async function fetchTokenPrice({
  ...
  if (chainId === undefined || chainId == 1 || chainId == 43114) {
    ...
    const otherMarket: MARKETINFO | undefined = networkInfo.contractAddresses.otherMarkets?.find((m: MARKETINFO) =>
      isSameAddress(m.address, address)
    );
    if (
      otherMarket !== undefined &&
      (otherMarket.platform == MarketProtocols.Sushiswap || otherMarket.platform == MarketProtocols.TraderJoe)
    ) {
...
export async function fetchValuation(
  ...
  return {
    ...
    amount: price
      ...
      .dividedBy(new BigNumber(10).pow(networkInfo.decimalsRecord[amount.token.address]))
