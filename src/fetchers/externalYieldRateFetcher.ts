...
import { ONE_DAY, sushiswapSubgraphApi, traderJoeSubgraphApi } from '../constants';
import { NetworkInfo } from '../networks';
import { distributeConstantsByNetwork, getCurrentTimestamp, getCurrentTimestampLocal, isSameAddress } from '../helpers';
...
export const fetchAaveYield = async (underlyingAddress: string) => {
  const url = 'https://aave-api-v2.aave.com/data/markets-data/0xb53c1a33016b2dc2ff3653530bff1848a515c8c5';
...
export const fetchCompoundYield = async (yieldBearingAddress: string) => {
  ...
  const yieldInPercentage = await axios
    ...
    .then((data: any) => {
      const underlyingData = data.cToken.find((data: any) => data.token_address.toLowerCase() === yieldBearingAddress);
...
export const fetchSushiForkYield = async (poolAddress: string, chainId?: number): Promise<number> => {
  //chainId
...
export async function fetchXJOEYield(provider: providers.JsonRpcProvider, chainId?: number): Promise<number> {
  ...
  const joeValuelocked = await fetchValuation(
    new TokenAmount(new Token(JOEAddress, networkInfo.decimalsRecord[JOEAddress]), joeLocked.toString()),
    provider,
    chainId
  );
  return sevenDayTradingVolume
    .multipliedBy(365)
    .div(7)
    .multipliedBy(0.0005)
    .div(joeValuelocked.amount)
    .toNumber();
}
