...
export class Sdk {
  ...
  public async fetchValuations(amounts: TokenAmount[], chainId: number): Promise<CurrencyAmount[]> {
    ...
    const promises = indexRange(0, amounts.length).map(async (i: number) => {
      await fetchValuation(amounts[i], this.signer.provider, chainId).then((r: CurrencyAmount) => response[i] = r);
    })
  ...
  public async claimYields({
    ...
    return RedeemProxy.methods({signer: this.signer, provider: this.signer.provider ,chainId: this.chainId}).claimYields({yts, ots, lps, interestStakingPools, rewardStakingPools, tokensToDistribute});
  }
  ...
  public async estimateGasForClaimYields({
    ...
    return RedeemProxy.methods({signer: this.signer, provider: this.signer.provider ,chainId: this.chainId}).estimateGasForClaimYields({yts, ots, lps, interestStakingPools, rewardStakingPools, tokensToDistribute})
  }
}
