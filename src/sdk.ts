...
import { StakingPool, Token, TokenAmount } from './entities';
...
export class Sdk {
  ...
  public async claimYields({
    ...
    return RedeemProxy.methods({ signer: this.signer, provider: this.provider, chainId: this.chainId }).claimYields({
      yts,
      ots,
      lps,
      interestStakingPools,
  ...
  public async estimateGasForClaimYields({
    ...
    }).estimateGasForClaimYields({ yts, ots, lps, interestStakingPools, rewardStakingPools, tokensToDistribute });
  }
}
