import { rdiv, rmul, RONE, rpow } from './mathLib';
...
export function calcUnweightedRate(inTokenReserve: BN, outTokenReserve: BN, inTokenDecimal: number): BN {
  return BN.from(10)
    .pow(inTokenDecimal)
    .mul(outTokenReserve)
    .div(inTokenReserve);
...
export function calcAvgRate(inAmount: BN, outAmount: BN, inTokenDecimal: number): BN {
...
export function calcOtherTokenAmount(inTokenReserve: BN, outTokenReserve: BN, inAmount: BN): BN {
...
export function calcPriceImpact(idealRate: BN, actualRate: BN): BigNumber {
  const priceImpact: BigNumber = BigNumber.max(
    new BigNumber(0),
    new BigNumber(idealRate.sub(actualRate).toString()).div(idealRate.toString())
  );
  return priceImpact;
}
...
export function calcImpliedYield(p: BigNumber, daysLeft: BigNumber): BigNumber {
  return new BigNumber(
    Math.pow(ONE.plus(p.dividedBy(ONE.minus(p))).toNumber(), new BigNumber(365).dividedBy(daysLeft).toNumber()) - 1
  );
}

export function calcPrincipalFloat(principalPerYT: BN, ytDecimal: number, underlyingDecimal: number): BigNumber {
  return new BigNumber(principalPerYT.mul(BN.from(10).pow(ytDecimal)).toString()).div(
    new BigNumber(
      BN.from(10)
...
export function calcValuation(unitPrice: BigNumber, amount: BN, decimals: number): BigNumber {
  const valuation: BigNumber = unitPrice.multipliedBy(amount.toString()).dividedBy(
    BN.from(10)
      .pow(decimals)
      .toString()
  );
