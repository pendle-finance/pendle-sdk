...
export function calcOutAmountLp(
  ...
  const inAmountRemained: BN = inTokenReserve
    .mul(exactOutLp)
    .div(totalSupplyLp);
  const inAmountSwapped: BN = inAmountAfterFee.sub(inAmountRemained);
  const outAmountSwapped: BN = outTokenReserve
    .mul(exactOutLp)
    .div(totalSupplyLp);
  return {
    exactOutLp,
...
export function calcPriceImpact(idealRate: BN, actualRate: BN): BigNumber {
  const priceImpact: BigNumber = BigNumber.max(
    new BigNumber(0),
    new BigNumber(idealRate.sub(actualRate).toString()).div(
      idealRate.toString()
    )
  );
  return priceImpact;
...
export function calcShareOfPool(
  originalBalance: BN,
  deltaInBalance: BN
): BigNumber {
  return new BigNumber(deltaInBalance.toString()).div(
    deltaInBalance.add(originalBalance).toString()
  );
}
...
export function calcOutAmountToken(
  ...
  const outAmountTokenBeforeSwapFee: BN = outTokenReserve.sub(
    outTokenBalanceUpdated
  );

  const feePortion: BN = rmul(RONE.sub(nWeight), swapFee);
  const exactOutToken: BN = rmul(
    outAmountTokenBeforeSwapFee,
    RONE.sub(feePortion)
  );

  const inAmountSwapped: BN = inLp.mul(inTokenReserve).div(totalSupplyLp);
  const outAmountSwapped: BN = outAmountTokenBeforeSwapFee.sub(
    inLp.mul(outTokenReserve).div(totalSupplyLp)
  );
...
export function calcReserveUSDValue(
  baseTokenAmount: BN,
  ...
  return new BigNumber(
    ...
    .div(Math.pow(10, baseTokenDecimal).toString());
}

export function calcSwapFeeAPR(
  volume: BigNumber,
  swapFee: BN,
  protocolFee: BN,
  liquidity: BigNumber
): BigNumber {
  return new BigNumber(rmul(swapFee, RONE.sub(protocolFee)).toString())
    .multipliedBy(volume)
    .multipliedBy(365)
    .dividedBy(new BigNumber(RONE.toString()).multipliedBy(liquidity));
}

export function calcTokenPriceByMarket(
  knownPrice: BigNumber,
  rate: BN,
  otherDecimal: number
): BigNumber {
  return knownPrice
    .multipliedBy(rate.toString())
    .dividedBy(Math.pow(10, otherDecimal));
}

export function calcPrincipalForSLPYT(
  exchangeRate: BN,
  principalDecimal: number
): BN {
...
export function calcImpliedYield(p: BigNumber, daysLeft: BigNumber): BigNumber {
  return new BigNumber(
    Math.pow(
      ONE.plus(p.dividedBy(ONE.minus(p))).toNumber(),
      new BigNumber(365).dividedBy(daysLeft).toNumber()
    ) - 1
  );
}

export function calcPrincipalFloat(
  principalPerYT: BN,
  ytDecimal: number,
  underlyingDecimal: number
): BigNumber {
  return new BigNumber(
    principalPerYT.mul(BN.from(10).pow(ytDecimal)).toString()
  ).div(
    new BigNumber(
      BN.from(10)
        .pow(18 + underlyingDecimal)
        .toString()
    )
  );
}
...
export function calcValuation(
  ...
  const valuation: BigNumber = unitPrice
...
export function calcCompoundYield(
  ratio: BigNumber,
  frequency: number
): BigNumber {
