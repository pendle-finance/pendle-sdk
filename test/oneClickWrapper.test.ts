...
describe("One click wrapper", () => {
  ...
  it.only('send', async() => {
    ...
    // ), 0.01)
    const sim_res: SimulationDetails = await wrapper.methods({signer, provider: signer.provider, chainId}).simulateSingle(Action.stakeOTYT, new TokenAmount(ETHToken, decimalFactor(18)), 0.01);
