...
describe('One click wrapper', () => {
  ...
  beforeAll(async () => {
    ...
    yieldContract = new YieldContract(
      ...
      new Token('0xd82b9b055f79d1a244005406988f85ed970797ed', 18),
      EXP_2022.toNumber(),
      chainId
    );
  ...
  it.only('Simulate stake', async () => {
    const res: SimulationDetails = await wrapper
    ...
    const res1: SimulationDetails = await wrapper
      .methods(signer, chainId)
      .simulate(
        Action.stakeOTYT,
        new TokenAmount(
  ...
  it('apr', async () => {
    ...
    console.log('stakeOT', JSON.stringify(res, null, '  '));
    const res1 = await wrapper.methods(signer, chainId).apr(Action.stakeYT);
    console.log('stakeYT', JSON.stringify(res1, null, '  '));
    const res2 = await wrapper.methods(signer, chainId).apr(Action.stakeOTYT);
    console.log('stakeOTYT', JSON.stringify(res2, null, '  '));
  });

  it('send', async () => {
    const sim_res: SimulationDetails = await wrapper
      .methods(signer, chainId)
      .simulate(
        Action.stakeOT,
        new TokenAmount(
    ...
    const res = await wrapper
      .methods(signer, chainId)
      .send(Action.stakeOT, sim_res.transactions, 0.01);
