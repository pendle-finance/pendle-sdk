...
function getMainnetMarkets() {
  const PendleEthMarket = PendleMarket.find(
    '0x685d32f394a5F03e78a1A0F6A91B4E2bf6F52cfE',
    1
  );
  const ETHUSDCMarket = PendleMarket.find(
    '0x79c05Da47dC20ff9376B2f7DbF8ae0c994C3A0D0',
    1
  );
  const aUSDC2022Market = PendleMarket.find(
    '0x8315bcbc2c5c1ef09b71731ab3827b0808a2d6bd',
    1
  );
  const cDAI2022Market = PendleMarket.find(
    '0xb26c86330fc7f97533051f2f8cd0a90c2e82b5ee',
    1
  );
  const aUSDC2021Market = Market.find(
...
describe('Market', () => {
  ...
  it('PendleMarket.readMarketDetails', async () => {
    const marketDetails = await market
  ...
  it('PendleMarket.swapExactInDetails', async () => {
    const swapExactInDetails = await market
      .methods(signer, chainId)
      .swapExactInDetails(
        new TokenAmount(
          market.tokens[1],
  ...
  it('PendleMarket.swapExactOutDetails', async () => {
    const swapExactOutDetails = await market
      .methods(signer, chainId)
      .swapExactOutDetails(
        new TokenAmount(
          market.tokens[0],
  ...
  it.skip('PendleMarket.swapExactIn', async () => {
    const response = await market
      .methods(signer)
      .swapExactIn(0.01, dummyTokenAmount);
  ...
  it.skip('PendleMarket.swapExactOut', async () => {
    const response = await market
      .methods(signer)
      .swapExactOut(0.01, dummyTokenAmount);
    console.log(response);
  });

  it('PendleMarket.addDualDetails', async () => {
    const response = await market.methods(signer, chainId).addDualDetails(
      new TokenAmount(
        tokens.USDCToken,
        BN.from(10)
          .pow(11)
  ...
  it.skip('PendleMarket.addDual', async () => {
    const response = await market
      .methods(signer)
      .addDual([dummyTokenAmount, dummyTokenAmount], 0.001);
    console.log(response);
  });

  it('PendleMarket.addSingleDetails', async () => {
    const response = await market.methods(signer, chainId).addSingleDetails(
      new TokenAmount(
        tokens.USDCToken,
        BN.from(10)
          .pow(12)
  ...
  it.skip('PendleMarket.addSingle', async () => {
    const response = await market
      .methods(signer)
      .addSingle(dummyTokenAmount, 0.001);
    console.log(response);
  });
  ...
  it('PendleMarket.removeSingleDetails', async () => {
    const response = await market
      .methods(signer)
      .removeSingleDetails(0.05, tokens.USDCToken, 0.001);
  ...
  it.skip('PendleMarket.removeSingle', async () => {
    const response = await market
      .methods(signer)
      .removeSingle(0.5, dummyToken, 0.001);
    console.log(response);
  });

  it('Market.readMarketDetails', async () => {
    const sushiMarket = Market.find(
  ...
  it.only('redeem OT market LP rewards', async () => {
    ...
    const res = await UniForkMarket.methods(
      signer,
      chainId
    ).fetchClaimableRewardsFromOTMarkets(
      indexRange(1, 4).map((i: number) => {
        return Market.find(
          networkInfo.contractAddresses.otherMarkets![i].address,
          chainId
        );
      }),
