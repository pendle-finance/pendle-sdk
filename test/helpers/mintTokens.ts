import { Token, consts, Network, tokens } from "./Constants"
...
export async function convertToAaveV2TokenMainnet (token: Token, alice: Wallet, amount: BN) {
    const { lendingPool } = new Contract(consts.address[Network.Mainnet].AAVE_V2_LENDING_POOL_ADDRESS, LendingPool.abi, alice);
...
export async function mintXytAaveMainnet(token: Token, user: Wallet, amount: BN, expiry: BN, env: TestEnv): Promise<BN> {
